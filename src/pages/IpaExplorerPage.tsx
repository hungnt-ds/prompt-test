...
import { SoundCard } from "@/components/ipa/SoundCard";
    ...
    <div className="h-screen flex flex-col bg-slate-100 dark:bg-[#0b1120] text-slate-900 dark:text-slate-100">
      ...
      <header className="h-16 flex items-center justify-between gap-3 pl-16 pr-4 md:px-6 shrink-0 border-b border-slate-300/60 dark:border-slate-800/60">
        <div className="min-w-0">
          <h1 className="text-[15px] font-semibold text-slate-900 dark:text-slate-100 truncate">Học phát âm IPA</h1>
          <p className="hidden sm:block text-[11px] text-slate-500">Bảng phiên âm quốc tế tiếng Anh — nhấn để nghe</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-10">
          <section className="space-y-2">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">English IPA Explorer</h2>
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed max-w-2xl">
              Nhấn vào từng ký tự để nghe cách phát âm âm đó trong một từ ví dụ (dùng hệ thống giọng
              nói trên thiết bị của bạn). Nhấn biểu tượng <span className="text-slate-700 dark:text-slate-300">ⓘ</span> ở góc thẻ để xem
              chi tiết âm với nhiều từ luyện nghe hơn.
            </p>
            {!supported && (
              <div className="flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2 w-fit">
            ...
            return (
              <section key={key} className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-slate-600 font-bold">{sounds.length} âm</span>
      ...
      <footer className="shrink-0 border-t border-slate-300/60 dark:border-slate-800/60 bg-white dark:bg-slate-950/40 px-4 md:px-6 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-center gap-4">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">Last Sound</span>
          {lastSound ? (
            <button
              onClick={() => play(lastSound)}
              className="flex items-center gap-2 text-sm text-slate-800 dark:text-slate-200 hover:text-white transition-colors"
            >
              <Volume2 className="w-4 h-4 text-blue-600 dark:text-blue-400" />
              <span className={`font-bold ${CATEGORY_ACCENT[lastSound.category]}`}>/{lastSound.symbol}/</span>
              <span className="text-slate-500">— {lastSound.example}</span>
            </button>
          ) : (
            <span className="text-sm text-slate-400 dark:text-slate-600 italic">None</span>
          )}
        </div>
      </footer>
    </div>
